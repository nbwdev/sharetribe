...
*/
window.ST.renderAdminImagePlaceholder = function() {
...
*/
window.ST.adminImageUploadElementManager = function($container) {
  var empties = [];
  var uploadings = [];
  var previews = [];
...
window.ST.adminImageUploader = function(listings, opts) {
  var elementManager = ST.adminImageUploadElementManager($("#image-uploader-container"));
  var directUploadToS3 = false;
  ...
  imageRemoved.filter(maybeNeedsAddPlaceholder).onValue(function() {
    var rendered = renderLocalUploader();
    elementManager.addEmpty(rendered.element);
  });
  ...
  function renderUpload(additionalOptions) {
    var $element = window.ST.renderAdminImagePlaceholder("empty");
  ...
  function renderUploading(data) {
    var $element = window.ST.renderAdminImagePlaceholder();
    ...
    var fileSaved = Bacon.mergeAll(fileSavedLocal).map(JSON.parse);
