...
import css from './ShopButton.css';

const HOVER_COLOR_BRIGHTNESS = 80;

export default function ShopButton({ text, url, customColor, className, mobileLayoutOnly }) {
  const buttonText = `${text}`;
  const color = customColor || variables['--ShopButton_defaultColor'];
  ...
  return React.createElement('a', {
    className: classNames(className, 'ShopButton', css.button, { [css.responsiveLayout]: !mobileLayoutOnly }),
    href: url,
    title: text,
    onMouseOver: (e) => {
      if (!hasCSSFilters()) {
        e.currentTarget.getElementsByClassName('ShopButton_background')[0].style.backgroundColor = hoverColor; // eslint-disable-line no-param-reassign
      }
    },
    onMouseOut: (e) => {
      if (!hasCSSFilters()) {
        e.currentTarget.getElementsByClassName('ShopButton_background')[0].style.backgroundColor = color; // eslint-disable-line no-param-reassign
      }
    },
  }, [
    ...
    React.createElement('span', {
      className: classNames('ShopButton_background',
        { [css.backgroundContainer]: true }),
      style: { backgroundColor: color },
    }),

    React.createElement('span', {
      className: classNames('ShopButton_mobile',
        { [css.mobile]: true }),
      style: { color },
    }, buttonText),
    React.createElement('span', {
      className: classNames('ShopButton_desktop',
        { [css.desktop]: true }),
    }, buttonText),
  ]);
}
